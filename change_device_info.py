...
try:
    build_desc = fingerprint.split(":")[0] + "/" + fingerprint.split("/")[1] + ":" + ":".join(fingerprint.split(":")[1:])
except:
...
try:
    subprocess.run(f'adb push "{zip_name}" /sdcard/Download/{zip_name}', shell=True, check=True)
...
print("Muốn chỉnh model/brand/security_patch khác hoặc thêm spoof MAC/IMEI thì báo tao nhé!")